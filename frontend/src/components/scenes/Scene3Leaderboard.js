...
export default function Scene3Leaderboard() {
  ...
  // Calculate total times for each pilot
  const leaderboard = pilots.map(pilot => {
    ...
    stages.forEach(stage => {
      ...
      if (time) {
        // Parse time string (assumes format like 00:00.000)
    ...
    return {
      ...
      totalTime,
      completedStages,
      totalTimeFormatted: totalTime > 0 ? `${Math.floor(totalTime / 60)}:${(totalTime % 60).toFixed(3).padStart(6, '0')}` : '-'
    };
  }).sort((a, b) => {
    if (a.completedStages !== b.completedStages) return b.completedStages - a.completedStages;
    ...
    <div className="relative w-full h-full flex items-center justify-center p-8" data-testid="scene-3-leaderboard">
      <div className="w-full max-w-5xl">
        ...
        <div className="text-center mb-8">
          ...
          {currentStage && (
            <p className="text-zinc-400 text-xl mt-2">After {currentStage.name}</p>
          )}
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {leaderboard.map((pilot, index) => {
                const gap = leader && pilot.id !== leader.id && pilot.totalTime > 0
                  ? '+' + (pilot.totalTime - leader.totalTime).toFixed(3) + 's'
                  : pilot.totalTime > 0 ? 'LEADER' : '-';
